...
df_all["Basket"] = df_all["Basket"].apply(remove_punctuation)

def split_basket(basket_str):
    elements = basket_str.split(",")
    return [item.strip() for item in elements]

df_all["Basket"] = df_all["Basket"].apply(split_basket)

df = df_all.explode("Basket", ignore_index=False)



filtered_df = df.query("`Payment Method`== 'Voucher'")


# print(filtered_df.info())

# plt.bar("Name","count",  data=filtered_df)

# plt.show()

print(filtered_df["Basket"].value_counts())
